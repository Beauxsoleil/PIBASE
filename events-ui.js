export function initEventsUi({ db, collection, onSnapshot, query, orderBy }) {
  ...
  const start=()=>{
    ...
    let eventDocs=[],resultDocs=[],applicants=docsOf(window.__PIBASE_APPLICANTS__),activeExtra=null,routeOrigin=null,eventKey='',territoryKey='';
    ...
    function renderEvents(force=false){const key=[...eventDocs,...resultDocs].map(e=>[e.id,e.date,e.name,e.summary,e.location,e.poc,e.type,e.leads,e.appointments,e.qualified,e.contracts,e.resultNotes,e.start].join('|')).join('~');if(!force&&key===eventKey)return;eventKey=key;const now=today(),up=[],past=[];let leads=0,appts=0,qualified=0,totalEvents=eventDocs.length;for(const e of eventDocs){const d=parseDate(e.date);if(d)(d>=now?up:past).push(e);leads+=num(e.leads);appts+=num(e.appointments);qualified+=num(e.qualified)}for(const r of resultDocs){const row={name:r.summary||'iCloud event',date:r.start||'',type:'iCloud',location:r.location||'',poc:'',leads:r.leads,appointments:r.appointments,qualified:r.qualified,contracts:r.contracts,resultNotes:r.resultNotes};const d=parseDate(row.date);if(d)(d>=now?up:past).push(row);leads+=num(r.leads);appts+=num(r.appointments);qualified+=num(r.qualified);totalEvents++;}up.sort((a,b)=>(a.date||'').localeCompare(b.date||''));past.sort((a,b)=>(b.date||'').localeCompare(a.date||''));setText('eventsUpcomingCount',`${up.length} upcoming`);setText('eventTotalEvents',totalEvents);setText('eventTotalLeads',leads);setText('eventTotalAppointments',appts);setText('eventTotalQualified',qualified);const u=up.length?up.slice(0,6).map(e=>eventRow(e)).join(''):'<div class="events-empty">No upcoming recruiting events entered in PIBASE.</div>',r=past.length?past.slice(0,4).map(e=>eventRow(e,true)).join(''):'<div class="events-empty">No completed event results yet.</div>';const ue=document.getElementById('eventsUpcoming'),re=document.getElementById('eventsRecent');if(ue&&ue.innerHTML!==u)ue.innerHTML=u;if(re&&re.innerHTML!==r)re.innerHTML=r;}
    ...
    const receive=value=>{applicants=docsOf(value);if(territory.classList.contains('active'))renderTerritory();};if(window.__PIBASE_APPLICANTS__)receive(window.__PIBASE_APPLICANTS__);window.addEventListener('pibase:applicants-snapshot',e=>receive(e.detail));
    onSnapshot(query(collection(db,'events'),orderBy('date','asc')),snap=>{eventDocs=docsOf(snap);if(events.classList.contains('active'))renderEvents();},err=>{console.warn('Recruiting events unavailable',err);const el=document.getElementById('eventsUpcoming');if(el)el.innerHTML='<div class="events-empty">Unable to load recruiting events.</div>';});onSnapshot(collection(db,'eventResults'),snap=>{resultDocs=docsOf(snap);if(events.classList.contains('active'))renderEvents();},err=>{console.warn('Calendar event results unavailable',err);});
    ...
    function showNative(screen,name){events.classList.remove('active');territory.classList.remove('active');screen.classList.add('active');title.textContent=name;activeExtra=null;announce(screen.id,name);}
    ...
    document.addEventListener('keydown',e=>{
      ...
      else if(e.key==='ArrowUp'||e.key==='ArrowDown'){requestAnimationFrame(()=>{announce(document.querySelector('.screen.active')?.id||'',title.textContent);});}
    },true);
    window.addEventListener('resize',()=>{if(events.classList.contains('active'))renderEvents();else if(territory.classList.contains('active'))renderTerritory();});
  };
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
}
