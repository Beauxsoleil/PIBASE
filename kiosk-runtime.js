export function initKioskRuntime() {
  ...
  const state = {
    ...
    applicants: window.__PIBASE_APPLICANTS_READY__ ? (navigator.onLine ? 'ready' : 'cache') : 'loading',
    calendar: 'loading',
    errors: 0,
  ...
  style.textContent = `
    ...
    .pibase-boot-title{font-size:1rem;letter-spacing:.12em;text-transform:uppercase;color:var(--text,#E9E6DC);margin-bottom:6px}.pibase-boot-sub{font-size:.65rem;color:var(--text-muted,#8FA096)}
    ...
    #pibaseStatusStrip .dot{width:7px;height:7px;border-radius:50%;background:var(--teal,#4FA88A)}#pibaseStatusStrip.degraded .dot{background:var(--brass,#C6A15B)}#pibaseStatusStrip.offline .dot{background:var(--rust,#C1543E)}
    ...
    @media(max-width:1100px){.board{padding:14px 16px 50px!important}.board-header{grid-template-columns:minmax(225px,.9fr) minmax(260px,1.15fr) minmax(160px,.75fr)!important;gap:14px!important;margin-bottom:12px!important;padding-bottom:10px!important}.board-header .seal{width:46px!important;height:46px!important}.board-header h1{font-size:1.55rem!important}.verse-banner{font-size:.52rem!important}.board-header .count{font-size:.78rem!important}.clock{font-size:.72rem!important}.grid{grid-template-columns:repeat(auto-fill,minmax(285px,1fr))!important;gap:10px!important}.card{padding:12px 14px!important}.card-top .name{font-size:1rem!important}.meta-line{margin-bottom:9px!important}.today-layout{grid-template-columns:minmax(0,1.15fr) minmax(290px,.85fr)!important;gap:12px!important}.pipeline-layout{grid-template-columns:minmax(285px,.85fr) minmax(0,1.45fr)!important;gap:12px!important}.calendar-layout{grid-template-columns:repeat(7,minmax(0,1fr))!important;gap:5px!important}.day-column{padding:8px 6px!important}.calendar-event{padding:6px 0!important}.calendar-event-title{font-size:.68rem!important}.calendar-event-location{font-size:.58rem!important}.panel{padding:13px 14px!important}.pipeline-row{grid-template-columns:140px 38px minmax(0,1fr)!important;gap:8px!important}.pipeline-stage{font-size:.58rem!important}.mission-number{font-size:3rem!important}}
    @media(max-height:720px){.board{padding-top:10px!important}.board-header{margin-bottom:8px!important;padding-bottom:8px!important}.kiosk-hint{margin-top:4px!important;min-height:14px!important}.panel{padding-top:10px!important;padding-bottom:10px!important}.panel-head{padding-bottom:8px!important}.agenda-row,.action-row{padding:8px 0!important}.metric-strip{margin-top:8px!important}.mission-stats{margin-top:10px!important;gap:9px!important}}
  `;
  ...
  const installUi = () => {
    ...
    if (!document.getElementById('pibaseBoot')) {
      ...
      boot.id = 'pibaseBoot';
      boot.innerHTML = '<div class="pibase-boot-card"><div class="pibase-boot-title">PIBASE</div><div class="pibase-boot-sub">Starting kiosk…</div></div>';
  ...
  if (document.body) installUi(); else document.addEventListener('DOMContentLoaded', installUi, { once: true });
  ...
  function renderStatus() {
    ...
    if (!state.online) { strip.classList.add('offline'); label = state.applicants === 'cache' || state.applicants === 'ready' ? 'Offline · Cached data' : 'Offline · Waiting for cached data'; }
    else if (state.applicants === 'error') { strip.classList.add('degraded'); label = 'Online · Applicant sync error'; }
    else if (state.applicants !== 'ready') { strip.classList.add('degraded'); label = 'Online · Applicant sync pending'; }
    else if (state.calendar !== 'ready') { strip.classList.add('degraded'); label = state.calendar === 'error' ? 'Online · Calendar retry' : 'Online · Calendar syncing'; }
  ...
  function updateScreenIndicator() {
    ...
    const order = [['boardScreen','Applicant Board'],['todayScreen','Today'],['calendarScreen','Calendar'],['pipelineScreen','Pipeline / Mission'],['eventsScreen','Recruiting Events'],['territoryScreen','Territory Analytics']];
  ...
  window.addEventListener('offline', () => { state.online = false; renderStatus(); dismissBoot(); });
  window.addEventListener('pibase:firebase-status', e => { const value = e.detail?.state || 'ready'; state.applicants = value === 'cache' ? 'cache' : value === 'error' ? 'error' : 'ready'; renderStatus(); dismissBoot(); });
  window.addEventListener('pibase:applicants-snapshot', () => { state.applicants = state.online ? 'ready' : 'cache'; renderStatus(); dismissBoot(); });

  // Poll the plain shared state too, so readiness does not depend on event timing.
  const applicantProbe = setInterval(() => {
    if (window.__PIBASE_APPLICANTS_READY__ || Array.isArray(window.__PIBASE_APPLICANTS__)) {
      state.applicants = state.online ? 'ready' : 'cache';
      renderStatus();
      clearInterval(applicantProbe);
    } else if (window.__PIBASE_APPLICANTS_ERROR__) {
      state.applicants = 'error';
      renderStatus();
    }
  }, 750);
  setTimeout(() => clearInterval(applicantProbe), 30_000);
